...
export const GetSoftwareBuiltPage: React.FC<Props> = ({ onViewChange, onBack, breadcrumbLabel = 'CRMs and dashboards', onServicesClick }) => {
  ...
  const handleCheckboxChange = (option: string) => {
    setFormData(prev => {
      const current = prev.automations;
      if (current.includes(option)) {
        return { ...prev, automations: current.filter(i => i !== option) };
      } else {
        return { ...prev, automations: [...current, option] };
      }
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      has_crm: formData.useCrm === 'yes', // Convert to boolean
      current_crm: formData.currentCrmName,
      needs: formData.automations,
    ...
    <div className="min-h-screen w-full bg-[#000000] text-white relative overflow-x-hidden font-sans selection:bg-[#8A2BE2] selection:text-white pb-20">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-24 md:pt-32 flex flex-col items-center">
        ...
        <div className="text-center max-w-4xl mx-auto mb-12 md:mb-16 px-4">
          <h1 className="text-[28px] md:text-[42px] lg:text-[48px] font-extrabold leading-[1.15] tracking-tight mb-4 md:mb-6 drop-shadow-2xl">
            Ready for a <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8A2BE2] via-[#C82DFF] to-[#FF2DAA]" style={{ textShadow: '0 0 30px rgba(138, 43, 226, 0.4)' }}>CRM</span> that actually makes your life <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8A2BE2] via-[#C82DFF] to-[#FF2DAA]">easier?</span>
          </h1>
          <p className="text-[14px] md:text-[18px] text-white/65 font-normal max-w-2xl mx-auto leading-relaxed">
            This quick form helps us understand what you need. Takes just 30 seconds.
          </p>
        </div>
          ...
          >
            {isSubmitted ? (
               ...
               <div className="min-h-[400px] flex flex-col items-center justify-center text-center animate-[fadeIn_0.5s_ease-out_forwards]">
                  ...
                  <p className="text-gray-400 text-lg">We'll get in touch shortly.</p>
                  <button 
              ...
              <form onSubmit={handleSubmit} className="space-y-8 md:space-y-10">
                ...
                {/* SECTION B: CRM REQUIREMENTS */}
                <div className="space-y-6">
                  <h3 className="text-xs font-bold text-[#8A2BE2] uppercase tracking-widest mb-4">02. CRM Requirements</h3>
                  
                  {/* Use CRM? */}
                  <div className="space-y-3">
                     <label className="block text-[13px] md:text-[14px] text-gray-400 font-medium ml-1">Do you currently use a CRM or software?*</label>
                        ...
                        {['Yes', 'No'].map(opt => (
                           <label key={opt} className="flex-1 cursor-pointer">
                              <input 
                                ...
                                onChange={e => setFormData({...formData, useCrm: e.target.value})}
                                required 
                                disabled={isSubmitting}
                              />
                              <div className="
                  ...
                  {formData.useCrm === 'yes' && (
                    <div className="space-y-2 animate-[fadeIn_0.3s_ease-out]">
                       <label className="block text-[13px] md:text-[14px] text-gray-400 font-medium ml-1">If yes, what CRM/software do you use?</label>
                       <input 
                         type="text"
                         placeholder="e.g. Zoho, Salesforce, Excel"
                         className="w-full h-[44px] md:h-[50px] bg-[#0C0C0E] border border-white/10 rounded-xl px-4 text-white placeholder:text-gray-600 focus:border-[#8A2BE2]/50 focus:ring-1 focus:ring-[#8A2BE2]/50 outline-none transition-all"
                         value={formData.currentCrmName}
                         onChange={e => setFormData({...formData, currentCrmName: e.target.value})}
                         disabled={isSubmitting}
                       />
                    </div>
                  )}
                  ...
                  {/* Automations */}
                  <div className="space-y-3">
                     <label className="block text-[13px] md:text-[14px] text-gray-400 font-medium ml-1">What do you want to build or automate?*</label>
                        ...
                        {AUTOMATION_OPTIONS.map(opt => (
                           <label key={opt} className="cursor-pointer group">
                              <input 
                                ...
                                checked={formData.automations.includes(opt)}
                                disabled={isSubmitting}
                              />
                              <div className="
                              ...
                              ">
                                 <div className="w-5 h-5 rounded border border-white/20 flex items-center justify-center peer-checked:bg-[#8A2BE2] peer-checked:border-[#8A2BE2] shrink-0">
                                    {formData.automations.includes(opt) && <Check className="w-3.5 h-3.5 text-white" />}
                                 </div>
                                 {opt}
                              </div>
                           </label>
                  ...
                  <div className="space-y-3 relative">
                     <label className="block text-[13px] md:text-[14px] text-gray-400 font-medium ml-1">What is your main issue?*</label>
                     <div className="relative">
                        ...
                        >
                           <span className={formData.mainIssue ? "text-white" : "text-gray-500"}>
                             {formData.mainIssue || "Select an issue"}
                           </span>
                           <ChevronDown className={`w-4 h-4 text-gray-500 transition-transform ${issueOpen ? 'rotate-180' : ''}`} />
                        </button>
                           ...
                           <div className="absolute top-full left-0 right-0 mt-2 bg-[#1A1A1E] border border-white/10 rounded-xl shadow-xl z-20 overflow-hidden">
                              {ISSUE_OPTIONS.map(opt => (
                                 <div 
                                   key={opt}
                                   onClick={() => {
                                      setFormData({...formData, mainIssue: opt});
                                      setIssueOpen(false);
                                   }}
